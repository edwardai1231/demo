...
class FindFlights {
 ...
 public static void main(String args[]) {
   ...
   if (dbSys.equals("o")) {
    ...
    if (yesNo.equals("y")) {
     ...
     String sid = readEntry(in, "sid (site id): ");
      origin = readEntry(in, "Please input Origin:");
      destination = readEntry(in, "Please input Destination:");
  ...
  try {
   ...
   findFlights(conn, origin, destination);   //query part
 ...
 // Scanner in = null;
  ...
  try{ 
   rset = stmt.executeQuery("select f.flno, f.distance, f.arrives-f.departs "
   		+ "from flights f where f.origin='" + origin + "' AND f.destination='" + destination + "'");
